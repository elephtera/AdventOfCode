namespace AdventOfCode2021.Assignments.Day2
{
    ...
    {
        private IList<Tuple<SubmarineAction, int>> GetInputData()
        {
            var input = InputHandler.GetInputAsList(InputDay2.Input);
            return input;
        }
            ...
            foreach (var input in GetInputData())
            {
                switch (input.Item1)
                {
                    case SubmarineAction.forward:
                        horizontalPosition += input.Item2;
                        depth += aim * input.Item2;
                        break;
