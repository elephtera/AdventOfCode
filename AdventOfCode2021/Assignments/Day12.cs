namespace AdventOfCode2021.Assignments
{
    ...
    public class Day12 : IDay
    {
        public string Part1()
        {
            ...
            var result = map.Travel(new List<string>(), "start");
        ...
        public string Part2()
        {
            var input = InputHandler.GetInputAsStringList(Day12Input.Input);
            var map = new CaveMap(input);
            var result = map.TravelB(new List<string>(), "start", false);
            var nice = new List<string>();
            foreach(var item in result)
            {
                nice.Add(string.Join(",", item));
            }
            var my = string.Join(Environment.NewLine, nice);
            return result.Count.ToString();
        }
    }
}
